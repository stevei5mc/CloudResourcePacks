...
### **注意事项**
1. **插件在初次加载时有报错为正常现象，这时需要对配置文件进行配置即可解决，后续在正确配置的内容后如果有报错则需要及时反馈**
2. **使用本插件后需要对资源包进行分配，否则不能将资源包正常的发送给玩家的**
3. **`default_packs.yml`配置的是对全体玩家提供的资源包**
5. **`need_permission_packs.yml`配置的是需要指定权限才能够进行发送给玩家的资源包**
